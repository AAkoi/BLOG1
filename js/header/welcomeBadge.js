...
#profile_block > *:not(.profile-impossible) { position: relative; z-index: 1; }
    ...
    // 保持 profile IMPOSSIBLE 持续存在
    if (!window.__PROFILE_IMPOSSIBLE_OBSERVED__) {
      window.__PROFILE_IMPOSSIBLE_OBSERVED__ = true;
      const ensureProfileImpossible = () => {
        const host = document.querySelector('#profile_block');
        if (!host) return;
        if (!host.querySelector('.profile-impossible')) {
          const layer = document.createElement('div');
          layer.className = 'profile-impossible';
          layer.setAttribute('aria-hidden', 'true');
          layer.innerHTML = '<div class="im-wrap"><h1><span>I</span>M<span>POSSIBLE</span></h1></div>';
...
#profile_block > *:not(.profile-impossible) { position: relative; z-index: 1; }
      ...
      const mo = new MutationObserver(() => ensureProfileImpossible());
      if (document.body) mo.observe(document.body, { childList: true, subtree: true });
      ensureProfileImpossible();
    }
